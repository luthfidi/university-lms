import {
  ...
  import { ChevronRightIcon, CheckIcon, CloseIcon, InfoIcon } from '@chakra-ui/icons';
  import { useNavigate, useParams } from 'react-router-dom';
  import { MdAssignment, MdQuiz, MdSchool, MdCheckCircle, MdCancel, MdBarChart, MdPerson, MdGroup, MdDateRange, MdAccessTime, MdCircle } from 'react-icons/md';
  ...
  const quizResultData = {
    ...
    percentage: 90,
    grade: 'A',
    feedback: 'Excellent understanding of control structures. Keep up the good work!',
    questions: [
      {
        id: 'q1',
        questionNumber: 1,
        ...
        correctAnswer: 'opt3',
        selectedAnswer: 'opt3',
        explanation: 'Python does not have a switch-case statement like other languages such as C or Java. Python 3.10+ introduced the match-case statement which provides similar functionality.'
      },
      {
        id: 'q2',
        questionNumber: 2,
        ...
        correctAnswer: 'opt2',
        selectedAnswer: 'opt2',
        explanation: 'The continue statement skips the rest of the code inside the loop for the current iteration. When i equals 3, the print statement is skipped, so 3 is not printed.'
      },
      {
        id: 'q3',
        questionNumber: 3,
        ...
        correctAnswer: 'opt3',
        selectedAnswer: 'opt2',
        explanation: 'The break statement exits the loop immediately, terminating the loop completely. The continue statement skips the current iteration and moves to the next one.'
      },
      ...
      breakdown: [
        {
          ...
          score: 35,
          maxScore: 40,
          comments: 'Your program correctly implements most of the required functionality. However, there was a minor issue with input validation that should be addressed.'
        },
        {
          category: 'Code Quality',
          score: 28,
          maxScore: 30,
          comments: 'Very clean, well-structured code. Good use of functions and appropriate naming conventions.'
        },
        {
          category: 'Documentation',
          score: 15,
          maxScore: 15,
          comments: 'Excellent documentation. Your comments were clear and helpful, and your README file provided comprehensive instructions.'
        },
        {
          category: 'Testing',
          score: 12,
  ...
  const AssessmentResultPage = () => {
    ...
    const renderQuizResult = () => (
      <>
        ...
        >
          <CardBody>
            <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6}>
              <Box textAlign="center">
                ...
                <HStack justify="center" spacing={2} mt={1}>
                  <Heading size="2xl" color={
                    result.percentage >= 90 ? 'green.500' :
                    result.percentage >= 80 ? 'blue.500' :
                    result.percentage >= 70 ? 'yellow.500' :
                    result.percentage >= 60 ? 'orange.500' :
                    'red.500'
                  }>
                    {result.percentage}%
                  </Heading>
                ...
                <Text mt={1} fontWeight="medium">{result.earnedPoints}/{result.totalPoints} points</Text>
              </Box>
              ...
              <Box>
                <Text fontSize="sm" color="gray.500" mb={2}>Class Performance</Text>
                <HStack spacing={4} align="center" mb={3}>
                  <Stat size="sm">
                    <StatLabel fontSize="xs">Average</StatLabel>
                    <StatNumber fontSize="md">{result.statistics.classAverage}%</StatNumber>
                  </Stat>
                  <Stat size="sm">
                    <StatLabel fontSize="xs">Highest</StatLabel>
                    <StatNumber fontSize="md">{result.statistics.highestScore}%</StatNumber>
                  </Stat>
                </HStack>
        ...
        {/* Feedback */}
        <Heading size="md" mb={4}>Instructor Feedback</Heading>
        <Card variant="outline" mb={6}>
          <CardBody>
            <Text fontWeight="bold" mb={4}>Overall Feedback</Text>
            <Box p={4} bg="blue.50" borderRadius="md" mb={6}>
              <Text>{assignmentResultData.feedback.overall}</Text>
            </Box>
            
            <Text fontWeight="bold" mb={4}>Grading Breakdown</Text>
            <VStack spacing={4} align="stretch">
              {assignmentResultData.feedback.breakdown.map((category, index) => (
                <Box key={index}>
                  <HStack justify="space-between" mb={1}>
                    <Text fontWeight="medium">{category.category}</Text>
                    <Text>{category.score}/{category.maxScore} points</Text>
                  </HStack>
                  <Progress 
                    value={(category.score / category.maxScore) * 100} 
                    size="sm" 
                    colorScheme={
                      (category.score / category.maxScore) >= 0.9 ? 'green' :
                      (category.score / category.maxScore) >= 0.8 ? 'blue' :
                      (category.score / category.maxScore) >= 0.7 ? 'yellow' :
                      (category.score / category.maxScore) >= 0.6 ? 'orange' :
                      'red'
                    } 
                    mb={2}
                  />
                  <Text fontSize="sm">{category.comments}</Text>
                  {index < assignmentResultData.feedback.breakdown.length - 1 && <Divider mt={4} />}
                </Box>
              ))}
            </VStack>
          </CardBody>
        ...
        <Card variant="outline" mb={6}>
          <CardBody>
            <Heading size="md" mb={4}>Class Statistics</Heading>
            <SimpleGrid columns={{ base: 1, md: 2 }} spacing={6}>
              <Box>
                <Text fontWeight="bold" mb={2}>Score Distribution</Text>
              ...
              <VStack align="stretch" spacing={4}>
                ...
                <Stat>
                  ...
                  <StatHelpText>
                    Your score is {result.percentage - result.statistics.classAverage > 0 ? '+' : ''}
                    {result.percentage - result.statistics.classAverage}% from average
                  </StatHelpText>
                </Stat>
              </VStack>
            </SimpleGrid>
    ...
    return (
      <Box>
        ...
        >
          ...
          <BreadcrumbItem>
            <BreadcrumbLink href={`/lms/assessment/${result.id}`}>{result.title}</BreadcrumbLink>
          </BreadcrumbItem>
        ...
        >
          ...
          <Box flex="1">
            <HStack mb={1}>
              ...
              <Badge 
                colorScheme={
                  result.type === 'assignment' ? 'purple' : 
                  result.type === 'quiz' ? 'orange' : 
                  'red'
                }
              >
            ...
            <Heading size="lg" mb={1}>{result.title} - Results</Heading>
          </Box>
        </Flex>
